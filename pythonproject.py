...
def save():
    con = connect(db='so', user='root', passwd='chandana99#', host='localhost')
    cur = con.cursor()
    name = n1.get()
    account = int(a1.get())
    amount = int(am1.get())
    ...
    cur.execute("SELECT amount FROM emp3 WHERE account=%s", (account,))
    row = cur.fetchone()
    if row:
        balance = int(row[0])
    else:
        cur.execute("insert into emp3 values (%s, %s, %s, %s)", (name, amount, account, ifsc))
        
        balance = 0

    # Update the balance and save the deposit transaction in the database
    new_balance = balance + amount
    cur.execute("UPDATE emp3 SET amount=%s WHERE account=%s", (new_balance, account))
    # cur.execute("INSERT INTO transactions (account, transaction_type, amount, balance) VALUES (%s, 'deposit', %s, %s)", (account, amount, new_balance))

    # Commit the transaction and close the connection
    con.commit()
    con.close()

    # Display a message box with the updated balance
    msg.showinfo('Information', f'Deposit of {amount} saved. New balance is {new_balance}.')
...
def withdraw():
    ...
    am1.pack()
    Label(withdraw_screen, text="").pack()
    Button(withdraw_screen, text="withdraw", width=10, height=1, bg="red", command = withdraw_draw).pack()
    Label(withdraw_screen, text="").pack()
...
def withdraw_draw():
    ...
    # Check if account has sufficient balance to withdraw
    cur.execute(f"SELECT amount FROM emp3 WHERE account={account} AND ifsc={ifsc}")
    result = cur.fetchone()
    if not result:
        msg.showerror('Error', 'Account does not exist or invalid IFSC')
    else:
        balance = result[0]
        if balance < amount:
            ...
            # Update the balance
            new_balance = balance - amount
            cur.execute(f"UPDATE emp3 SET amount={new_balance} WHERE account={account} AND ifsc={ifsc}")
            con.commit()
            msg.showinfo('Information', 'Amount Withdrawn')
...
def balance():
    ...
    account_lable = Label(balance_screen, text="Account number * ")
    account_lable.pack()
    a1 = Entry(balance_screen)
    a1.pack()
    Button(balance_screen,text="Show_Balance", height="2", width="10",fg='black',bg='orange', command=show_bel).pack()
def show_bel():
    con = connect(db='so', user='root', passwd='chandana99#', host='localhost')
    cur = con.cursor()
    # name = n1.get()
    ...
    # amount = int(am1.get())
    # ifsc = int(if1.get())
    i = cur.execute(f"SELECT amount FROM emp3 WHERE account={account}")
    amount = cur.fetchone()[0]
    Label(balance_screen, text=f"Your account Balence is ₹ {amount}").pack()

    # if i >= 1:
        
    #     msg.showinfo('Information', f'Current balance for account {account}: {amount}')
    # else:
    #     msg.showerror('Error', f'No account found with number {account}')
    con.close()
